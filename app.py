...
# Web App Title
st.title("Conference Schedule Maker")
...
st.markdown("""
    **Welcome to the Conference Schedule Maker!**  
    ...
    ### Column Descriptions:
    - **Theme**: The category or department of the presentation (e.g., Arts, Biology, Computer Science).
    - **Title**: The title of the presentation.
    - **Presenter(s)**: The name(s) of the presenter(s), separated by commas if there are multiple presenters.
    - **Faculty Mentor**: The name(s) of the faculty mentor(s) associated with the presentation.

    **Note**: Ensure all four columns are included and correctly labeled to avoid errors in the scheduling process.
""")
            ...
            for i in range(num_sections):
                ...
                with col1:
                    start = st.time_input(f"Start time:", key=f"start_{i}",
                                        ...
                                        datetime.strptime("11:00 AM", "%I:%M %p").time())
                with col2:
                    end = st.time_input(f"End time:", key=f"end_{i}",
                                      value=datetime.strptime("2:00 PM", "%I:%M %p").time() if i == 0 else 
...
# Poster Session Maker
            ...
            # Ask for number of sections  
            num_sections = st.number_input("Number of poster sections:", min_value=1, max_value=5, value=2)
            ...
            for i in range(num_sections):
                ...
                with col1:
                    start = st.time_input(f"Start Time:", key=f"poster_start_{i}",
                                        value=datetime.strptime("10:00 AM", "%I:%M %p").time()  if i == 0 else 
                                        datetime.strptime("11:30 AM", "%I:%M %p").time())
                with col2:
                    end = st.time_input(f"End Time:", key=f"poster_end_{i}",
                                      value=datetime.strptime("1:00 PM", "%I:%M %p").time() if i == 0 else 
            ...
            for idx, group in enumerate(poster_groups):
                start_time = datetime.combine(datetime.today(), poster_sections[idx]['start'])
                end_time = datetime.combine(datetime.today(), poster_sections[idx]['end'])

                # Format full time slot string
                slot = f"{start_time.strftime('%I:%M %p')} - {end_time.strftime('%I:%M %p')}"

                for row_offset, row_idx in enumerate(group.index):
                    df.at[row_idx, 'Poster Section'] = poster_sections[idx]['name']
                    df.at[row_idx, 'Time Slot'] = slot
                    df.at[row_idx, 'Poster Number'] = row_offset + 1  # Number within this section

            final_df = df[['Poster Section', 'Poster Number', 'Time Slot', 'Theme', 'Title', 'Presenter(s)', 'Faculty Mentor']]
            st.write("**Poster Schedule Preview:**")
            st.dataframe(final_df.head(20))
            st.write(f"Total posters scheduled: {len(final_df)}")

            csv = final_df.to_csv(index=False).encode('utf-8')
            st.download_button("Download Poster Schedule CSV", csv, "poster_schedule.csv", "text/csv")